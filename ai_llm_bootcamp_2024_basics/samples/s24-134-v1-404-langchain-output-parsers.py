...
#Language models respond to our questions with plain text, 
# but sometimes we would like to have those answers in a different format, 
# like a JSON dictionary or a XML document. In order to achieve that, we use the Output Parsers.

#Example WITHOUT output parsers

llm = init_llm(llmmodel=LlmModel.GEMINI)
